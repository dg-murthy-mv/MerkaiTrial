...
namespace MerkaiTrial.Admin.Web.Pages.Reports.Pipeline
{
    public class IndexModel : ReportBaseModel
    {
        ...
        public PipelineSummaryReportDto? Data { get; private set; }
        public string DataJson { get; private set; } = "null";

        public async Task OnGetAsync()
        {
            await InitAsync("pipeline");
            try
            {
                Data = await ReportService.GetPipelineSummaryAsync(FromDate, ToDate, Vertical);
        ...
        {
            var bytes = await ReportService.ExportPipelineAsync(FromDate, ToDate, Vertical);
            return ExcelFile(bytes, "PipelineSummary");
        }
    }
}
