...
namespace MerkaiTrial.Admin.Web.Pages.Reports.SalesFunnel
{
    public class IndexModel : ReportBaseModel
    {
        ...
        public SalesFunnelReportDto? Data { get; private set; }
        public string DataJson { get; private set; } = "null";

        public async Task OnGetAsync()
        {
            await InitAsync("funnel");
            try
            {
                Data = await ReportService.GetSalesFunnelAsync(FromDate, ToDate, Vertical);
        ...
        {
            // Sales funnel is chart-only, no Excel export needed
