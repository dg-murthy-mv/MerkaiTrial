...
namespace MerkaiTrial.Admin.Web.Pages.Reports.ByVertical
{
    public class IndexModel : ReportBaseModel
    {
        ...
        public RevenueByVerticalReportDto? Data { get; private set; }
        public string DataJson { get; private set; } = "null";

        public async Task OnGetAsync()
        {
            await InitAsync("vertical");
            try
            {
                Data = await ReportService.GetRevenueByVerticalAsync(FromDate, ToDate, Vertical);
        ...
        {
            var bytes = await ReportService.ExportRevenueByVerticalAsync(FromDate, ToDate, Vertical);
            return ExcelFile(bytes, "RevenueByVertical");
        }
    }
}
